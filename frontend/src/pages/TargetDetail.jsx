...
import { fmtMs, statusMeta, timeAgo } from "../lib/utils-sp";
...
export default function TargetDetail() {
  ...
  const target = useMemo(
    () => flattenTargets(mockTree).find((t) => t.id === id),
    [id]
  );

  const series = useMemo(
    () => (target ? generateSeries(target, range) : []),
    [target, range, seed]
  );
  const stats = useMemo(() => (series.length ? computeStats(series) : null), [
    series,
  ]);
  ...
  return (
    <div className="mx-auto max-w-[1400px] p-4 lg:p-6">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <StatPill label="Current" value={fmtMs(stats.current)} color="#22d3ee" />
        <StatPill
          label="Loss"
          value={`${stats.currentLoss}%`}
          color={lossColor(stats.currentLoss)}
        />
        <StatPill label="Avg" value={fmtMs(stats.avg)} />
        <StatPill label="Min" value={fmtMs(stats.min)} color="#4ade80" />
        <StatPill label="Max" value={fmtMs(stats.max)} color="#fb923c" />
        <StatPill label="Avg Loss" value={`${stats.avgLoss}%`} color="#a855f7" />
      </div>
      ...
      <AlertDialog open={delOpen} onOpenChange={setDelOpen}>
        <AlertDialogContent className="glass border-border/60">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the target and its history. This action cannot be
              undone (demo — no data is actually deleted yet).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
